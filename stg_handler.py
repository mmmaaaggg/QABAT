...
import numpy as np
import pandas as pd
from md_agent import MdAgentBase
from config import Config, PeriodType, RunMode, ContextKey, Direction, BacktestTradeMode
import logging
...
from td_agent import RealTimeTraderAgent, BacktestTraderAgent
from backend.fh_utils import try_2_date
from backend.orm import StgRunInfo, PosStatusInfo
...
class StgHandlerBase(Thread, ABC):
    ...
    @staticmethod
    def factory(stg_class_obj: 'StgBase', strategy_params, md_agent_params_list, run_mode: RunMode, **run_mode_params):
        """
        建立策略对象
        建立数据库相应记录信息
        更加运行模式（实时、回测）：选择相应的md_agent以及trade_agent
        :param stg_class_obj: 
        ...
        with Config.with_db_session(Config.get_db_engine(Config.DB_SCHEMA_QABAT)) as session:
        ...
        if run_mode == RunMode.Backtest:
            ...
            raise ValueError('run_mode %d error' % self.run_mode)
        # 初始化策略实体，传入参数
        stg_base = stg_class_obj(**strategy_params)
        # 设置策略交易接口 trade_agent，这里不适用参数传递的方式而使用属性赋值，是因为stg子类被继承后，参数主要用于设置策略所需各种参数使用
    ...
    def stg_run_ending(self):
        ...
        with Config.with_db_session(schema_name=Config.DB_SCHEMA_QABAT) as session:
            ...
            # sql_str = StgRunInfo.update().where(StgRunInfo.c.stg_run_id == self.stg_run_id).values(dt_to=datetime.now())
...
class StgHandlerBacktest(StgHandlerBase):
    ...
    def __init__(self, stg_run_id, stg_base: 'StgBase', md_period_agent_dic, date_from, date_to, **kwargs):
        ...
        for period, md_agent in self.md_period_agent_dic.items():
            his_df = md_agent.load_history(date_from, date_to, load_md_count=0)
            if his_df is None:
                continue
            self.backtest_his_df_dic[period] = his_df
            self.logger.debug('加载 %s 回测数据 %d 条记录', period, his_df.shape[0])
    ...
    def run(self):
        ...
        try:
            ...
            period_dt_idx_dic = {}
            for period, backtest_his_df in self.backtest_his_df_dic.items():
                df_len = backtest_his_df.shape[0]
                action_day_s = backtest_his_df['ActionDay']
                action_time_s = backtest_his_df['ActionTime']
                # Tick 数据 存在 ActionMillisec 记录秒以下级别数据
                if period == PeriodType.Tick:
                    action_milsec_s = backtest_his_df['ActionMillisec']
                    dt_idx_dic = {}
            ...
            for row_num in range(period_idx_df.shape[0]):
                ...
                for period, idx_list in period_idx_s.items():
                    if all(np.isnan(idx_list)):
                        continue
                    backtest_his_df = self.backtest_his_df_dic[period]
                    for idx_row in idx_list:
                        # TODO: 这里存在着性能优化空间 DataFrame -> Series -> dict 效率太低
                        md = backtest_his_df.ix[idx_row].to_dict()
...
class StgBase:
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger()
        # self._on_period_md_event_dic = {
        #     PeriodType.Tick: self.on_tick,
        #     PeriodType.Min1: self.on_min1,
        #     PeriodType.Hour1: self.on_hour1,
        #     PeriodType.Day1: self.on_day1,
        #     PeriodType.Week1: self.on_week1,
        #     PeriodType.Mon1: self.on_month1,
        # }
        # self._on_period_prepare_event_dic = {
        #     PeriodType.Tick: self.on_prepare_tick,
        #     PeriodType.Min1: self.on_prepare_min1,
        #     PeriodType.Hour1: self.on_prepare_hour1,
        #     PeriodType.Day1: self.on_prepare_day1,
        #     PeriodType.Week1: self.on_prepare_week1,
        #     PeriodType.Mon1: self.on_prepare_month1,
        # }
        self._on_period_event_dic = {
            PeriodType.Tick: EventHandlersRelation(PeriodType.Tick, self.on_prepare_tick, self.on_tick, pd.DataFrame),
            PeriodType.Min1: EventHandlersRelation(PeriodType.Min1, self.on_prepare_min1, self.on_min1, pd.DataFrame),
            PeriodType.Hour1: EventHandlersRelation(PeriodType.Hour1, self.on_prepare_hour1, self.on_hour1, pd.DataFrame),
            PeriodType.Day1: EventHandlersRelation(PeriodType.Day1, self.on_prepare_day1, self.on_day1, pd.DataFrame),
            PeriodType.Week1: EventHandlersRelation(PeriodType.Week1, self.on_prepare_week1, self.on_week1, pd.DataFrame),
            PeriodType.Mon1: EventHandlersRelation(PeriodType.Mon1, self.on_prepare_month1, self.on_month1, pd.DataFrame),
        }
    ...
    def close_long(self, instrument_id, price, vol, offset_flag=ApiStruct.OF_Close):
        self.trade_agent.close_long(instrument_id, price, vol, offset_flag)
    ...
    def close_short(self, instrument_id, price, vol, offset_flag=ApiStruct.OF_Close):
        self.trade_agent.close_short(instrument_id, price, vol, offset_flag)
    ...
    def get_position(self, instrument_id) -> dict:
        ...
        :return: 
        """
        return self.trade_agent.get_position(instrument_id)
...
class MACroseStg(StgBase):
    ...
    def on_min1(self, md_df, context):
        ...
        if self.ma5[-2] < self.ma10[-1] and self.ma5[-1] > self.ma10[-1]:
        ...
        elif self.ma5[-2] > self.ma10[-1] and self.ma5[-1] < self.ma10[-1]:
            position_date_pos_info_dic = self.get_position(instrument_id)
            no_target_position = True
            if position_date_pos_info_dic is not None:
                for position_date, pos_info in position_date_pos_info_dic.items():
...
if __name__ == '__main__':
    ...
    md_agent_params_list = [{
        ...
        'instrument_id_list': ['ag1712'],  # ['jm1711', 'rb1712', 'pb1801', 'IF1710'],
